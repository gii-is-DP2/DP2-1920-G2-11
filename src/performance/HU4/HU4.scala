...
class HU4 extends Simulation {
	...
	object ShowVaccine {
		val showVaccine = exec(http("ShowVaccine")
			.get("/owners/*/pets/*/sicknesses/%7BsicknessId%7D/vaccines/1")
			.headers(headers_0))
		.pause(5)
}

	val scnHU4 = scenario("HU4").exec(Home.home,
									  Login.login,
									  ShowVaccine.showVaccine)

	setUp(
		scnHU4.inject(rampUsers(8000) during (100 seconds))
