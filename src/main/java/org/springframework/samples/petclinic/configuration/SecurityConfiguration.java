...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/resources/**", "/webjars/**", "/h2-console/**").permitAll().antMatchers(HttpMethod.GET, "/", "/oups").permitAll().antMatchers("/users/new").permitAll().antMatchers("/admin/**").hasAnyAuthority("admin")
			.antMatchers("/owners/**").authenticated().antMatchers("/vets/**").authenticated().antMatchers("/sicknesses/delete/**").hasAnyAuthority("veterinarian").antMatchers("/admin/medicines/**").hasAnyAuthority("admin").antMatchers("/owner/medicines/**").hasAnyAuthority("owner").antMatchers("/owner/medicine/**").hasAnyAuthority("owner").antMatchers("/clinics/**").authenticated().antMatchers("/products/**").authenticated()
