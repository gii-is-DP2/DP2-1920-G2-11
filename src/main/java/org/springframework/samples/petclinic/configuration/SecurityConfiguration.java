...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/resources/**", "/webjars/**", "/h2-console/**").permitAll().antMatchers(HttpMethod.GET, "/", "/oups").permitAll().antMatchers("/users/new").permitAll().antMatchers("/admin/**").hasAnyAuthority("admin")
			...
			.antMatchers("/vets/newVaccine").hasAnyAuthority("veterinarian"/* , "admin" */).antMatchers("/clinics").authenticated()

			.antMatchers("/clinics/**").hasAnyAuthority("owner", "admin").antMatchers("/vets/delete/**").hasAnyAuthority("veterinarian", "admin").antMatchers("/vets/newSickness").hasAnyAuthority("veterinarian").antMatchers("/clinics/new")
			.hasAnyAuthority("admin").antMatchers("/clinics/save").hasAnyAuthority("admin").antMatchers("/vets/**").hasAnyAuthority("veterinarian", "admin", "owner").antMatchers("/owners/**").authenticated().antMatchers("/sicknesses/delete/**")
			.hasAnyAuthority("veterinarian").antMatchers("/admin/medicines/**").hasAnyAuthority("admin").antMatchers("/owner/medicines/**").hasAnyAuthority("owner").antMatchers("/owner/medicine/**").hasAnyAuthority("owner")
